...
/*
$sql = "SELECT orderId FROM `barcode`";
$results = $db_mysql->query($sql);
$results = $results->fetchAll(PDO::FETCH_COLUMN); // FETCH_ASSOC FETCH_COLUMN FETCH_KEY_PAIR FETCH_NUM

$orderIDS_lookup = array_flip($results);

$json_dispatched = file('data/json_dispatched.php', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);

$tmp = [];
foreach ($json_dispatched as $line) {
    $arr = json_decode($line, true);
    
    if (isset($orderIDS_lookup[$arr['orderID']])) {
        $tmp[] = $line;
    }
}

echo '<pre style="background:#111; color:#b5ce28; font-size:11px;">'; print_r($json_dispatched); echo '</pre>';
echo '<pre style="background:#111; color:#b5ce28; font-size:11px;">'; print_r($orderIDS_lookup); echo '</pre>'; die();
*/

$sql ="CREATE TABLE `undispatched_orderIDs` (
    `orderId`       varchar(42) NOT NULL,
    `platform`      char(2) NOT NULL,
    PRIMARY KEY(`orderId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;";

echo "Create dispatched_orders table.<br>";
$db_mysql->exec($sql);

die();

try {
    if ($db_mysql->exec("DROP TABLE IF EXISTS dispatched_orders")) {
        echo "Deleted dispatched_orders table.<br>";
    }
    
    $sql ="CREATE TABLE `dispatched_orders` (
        `orderId`       varchar(42) NOT NULL,
        `platform`      char(2) NOT NULL,
        `ts` int(10) NOT NULL,
        -- `ts` int(10) NOT NULL DEFAULT current_timestamp(),
        PRIMARY KEY(`orderId`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";

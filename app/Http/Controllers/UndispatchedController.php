    ...
    */
    ...
    )
    {
        ...
        */
        ...
        //DEBUG
        // $res = $dbQueryService->missingBarcodeOrders();
        // echo '<pre style="background:#111; color:#b5ce28; font-size:11px;">'; print_r($res); echo '</pre>'; die();
        
        $data['pl_lkup'] = $elixirVars->platformLookup()['pl_lkup'];
        
        
        $view_name = 'undispatched';
        $args = [
            'op' => $view_name,
            // 'op' => $view_name, // 'undispatched' OR 'invoice'
            'data' => $dbQueryService->getOrders($data),
            'map_lookup' => $elixirVars->platformLookup(),
        ];
        
        $view_data = $createDataService->createDataArray($args);
        
        // echo '<pre style="background:#111; color:#b5ce28; font-size:11px;">'; print_r($view_data); echo '</pre>'; die();
        
        usort($view_data['tbl_body'], function($a, $b) {
            return $a['ts'] <=> $b['ts'];
        });
